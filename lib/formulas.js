...
formulas.parseString = function(string){
  ...
  */
  ...
  var regexStr = /((("([^"]+)?")|((\w+\!)?\$?[A-Z]\$?\d+)(\:(\w+\!)?\$?[A-Z]\$?\d+)?|((\w+\!)?([CR](?:\[-?)?\d+\]?\:?[CR](?:\[-?)?\d+\]?)([: ][CR](?:\[-?)?\d+\]?[CR](?:\[-?)?\d+\]?)?)|(\-?[\d\.]+)|(TRUE|FALSE)|([A-Z][A-Z0-9]+\(.*\)))(\+|\&|\-|\*|\/|\^|\>|\<|\<\=|\>\=|\=|\<\>)?)+(\s+)?(?:$|,)/g
  var parsedArgs = []
  args.replace(regexStr, function(m) {
    ...
    */
    var regexStr = /(("([^"]+)?")|((\w+\!)?\$?[A-Z]\$?\d+)(\:(\w+\!)?\$?[A-Z]\$?\d+)?|((\w+!)?([CR](?:\[-?)?\d+\]?\:?[CR](?:\[-?)?\d+\]?)([: ][CR](?:\[-?)?\d+\]?[CR](?:\[-?)?\d+\]?)?)|(\-?[\d\.]+)|(TRUE|FALSE)|([A-Z][A-Z0-9]+\(.*\))|(\w+))(?:\s+)?($|\-|\+|\*|\/|\&|\^|\=)/g
    var atoms = []
...
formulas.format = function(formulaObj, cell, row){
  ...
  if (localName){
    ...
    return parts.join(formulas.sep+" ")
  }
