...
formulas.parseString = function(string){
  ...
  */
  ...
  var regexStr = /(("[^"]+"((\s+)?&(\s+)?("[^"]+")){0,99})|((\w+!)?([CR](?:\[-?)?\d+\]?\:?[CR](?:\[-?)?\d+\]?)(:[CR](?:\[-?)?\d+\]?[CR](?:\[-?)?\d+\]?)?)|([\d\.\+\/\-\*\s]+)|(TRUE|FALSE)|([A-Z][A-Z0-9]+\(.*\))|(\w+))(\s+)?(?:$|,)/g
//  var regexStr = /(?:(,(\s+)?)|^)(.+)(\s+)?(?:$|,)/g
  var parsedArgs = []
...
/* Create a full, localized string from a formulaObject, eg:
   ...
   cell and row arguments are required to properly translate
...
formulas.format = function(formulaObj, cell, row){
  ...
  for (var arg of formulaObj.args){
    if (typeof arg == "object"){
      parts.push(formulas.format(arg, cell, row))
    } else {
      ...
      else if (/(([CR](?:\[-?)?\d+\]?\:?[CR](?:\[-?)?\d+\]?)(:[CR](?:\[-?)?\d+\]?[CR](?:\[-?)?\d+\]?)?)/.exec(arg)){
        if ((typeof cell !== "undefined") && (typeof row !== "undefined")){
          console.log("FORMEL: ", arg)
          // R1, R99
          arg = arg.replace(/R\d+/g, function(m) {
          ...
          // C1
          arg = arg.replace(/C\d+/g, function(m) {
          ...
          arg = arg.replace(/(\d+)([A-Z])/g, function(i, r, c){return "" + c + r})
          console.log(arg)
        }
      }
      parts.push(arg)
