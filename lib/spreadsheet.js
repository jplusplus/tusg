const cache = require('memjs').Client.create()
...
module.exports = function(options){
  ...
  */
  spreadsheet.filter = function(text, filterOptions, cb){
    ...
    let cacheKey = key + crypto.randomBytes(20).toString('hex')
    var highlite = filterOptions.highlight.split(",")
    var class_ = null
    ...
    async.waterfall([
      ...
      function getData(step) {
        ...
        doc.getInfo(function(err, info) {
          ...
          sheet.getCells({
            'min-row': 1,
            'max-row': settings.defaultSpreadsheetHeight,
            'return-empty': true
          }, function( err, c ){
            cells = c
            cache.set(cacheKey, JSON.stringify(cells), {expires:TO}, function(err){
              if (err) {
      ...
      function parseData(step) {
        ...
        for (let cell of cells) {
          ...
          /* Highlite this cell? */
          var cellRef = String.fromCharCode(65 + col) + (row+1).toString()
          if (highlite.indexOf(cellRef) > -1){
            cellClass += " highlight"
          }
          // Data
          data[row][col] = {
