...
module.exports = function(options){
  ...
  */
  spreadsheet.filter = function(text, options, cb){

    var key = Object.keys(options)[0]
    if (Object.keys(options).length > 1 && Object.keys(options)[1] === "bad"){
      var class_ = "bad"
    } else {
      var class_ = null
    }
    ...
    async.waterfall([
      ...
      function getHelpers(step){
        var step = step
        textFunctions = require("../lib/text-functions.js")(options.locale)

        formulas.init(options.software,
                      options.language,
                      options.locale, function(){
                        step()
                      })
      },
